...
const socialLinks = [
  { name: 'Facebook', icon: FaFacebook, href: 'https://www.facebook.com/share/1BVLsQdziT/?mibextid=wwXIfr', label: 'Unity Typing & Travels on Facebook' },
  { name: 'Instagram', icon: FaInstagram, href: 'https://www.instagram.com/unity_typing?stkn=MTU4bWN2cWc4bzRieA%3D%3D&utm_source=qr', label: 'Unity Typing on Instagram' },
  { name: 'TikTok', icon: FaTiktok, href: 'https://www.tiktok.com/@unity_typing?_r=1&_t=ZS-99t50HJREo9', label: 'Unity Typing on TikTok' },
]
...
function App() {
  ...
  return (
    <div className="site-shell">
      <div className="utility-bar"><span>UAE SERVICES &amp; TRAVEL SUPPORT</span><span className="follow-us"><span>Follow us</span>{socialLinks.map(({ name, icon: Icon, href, label }) => <a key={name} href={href} target="_blank" rel="noreferrer" aria-label={label}><Icon aria-hidden="true" /></a>)}<b>Unity Typing: @unity_typing</b><b>Unity Travels: @unity_travels_uae</b></span></div>
      ...
      {selectedService ? <ServiceDetail service={selectedService} onBack={closeDetail} /> : <main id="top">
        ...
        <section className="contact-section" id="contact"><div><p className="eyebrow"><span /> Your next move</p><h2>Have a question?<br /><em>Let’s talk.</em></h2><p className="contact-intro">Connect with the Unity team at the location that suits your enquiry.</p><div className="social-panel"><p className="eyebrow"><span /> Follow Us</p><div className="social-links">{socialLinks.map(({ name, icon: Icon, href, label }) => <a key={name} href={href} target="_blank" rel="noreferrer" aria-label={label}><strong><Icon aria-hidden="true" /></strong><span>{name}</span></a>)}</div><p className="social-handle">Unity Travels: @unity_travels_uae <small>No separate Unity Travels URL has been supplied.</small></p></div></div><div className="contact-panel"><div className="contact-list">{contactLocations.map((contact) => <div className="contact-location" key={contact.division}><span>{contact.division}</span><p>{contact.location}</p><a className="contact-phone" href={`tel:${contact.phone}`}>{contact.displayPhone}</a><div className="contact-actions"><a href={`tel:${contact.phone}`}><FaPhone aria-hidden="true" /><span>Call</span></a><a href={`https://wa.me/${contact.whatsapp}`} target="_blank" rel="noreferrer"><FaWhatsapp aria-hidden="true" /><span>WhatsApp</span></a></div></div>)}</div><small>No email, opening hours or map details have been supplied yet.</small></div></section>
      </main>}
