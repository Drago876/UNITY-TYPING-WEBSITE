  ...
  return (
    <div className="site-shell">
      ...
      {selectedService ? <ServiceDetail service={selectedService} onBack={closeDetail} /> : <main id="top">
        ...
        <section className="contact-section" id="contact"><div><p className="eyebrow"><span /> Your next move</p><h2>Have a question?<br /><em>Let’s talk.</em></h2><p className="contact-intro">Connect with the Unity team at the location that suits your enquiry.</p><div className="social-panel"><p className="eyebrow"><span /> Follow Us</p><div className="social-links">{socialLinks.map((social) => <a key={social.name} href={social.href} target="_blank" rel="noreferrer" aria-label={social.label}><strong><social.icon /></strong><span>{social.name}</span><span>{social.name}</span></a>)}</div><p className="social-handle">Unity Travels: @unity_travels_uae <small>No separate Unity Travels URL has been supplied.</small></p></div></div><div className="contact-panel"><div className="contact-list">{contactLocations.map((contact) => <div className="contact-location" key={contact.division}><span>{contact.division}</span><p>{contact.location}</p><a className="contact-phone" href={`tel:${contact.phone}`}>{contact.displayPhone}</a><div className="contact-actions"><a href={`tel:${contact.phone}`}>Call</a><a href={`https://wa.me/${contact.whatsapp}`} target="_blank" rel="noreferrer">W<div className="contact-actions"><a href={`tel:${contact.phone}`}><FaPhone /> Call</a><a href={`https://wa.me/${contact.whatsapp}`} target="_blank" rel="noreferrer"><FaWhatsapp /> WhatsApp</a></div>hatsApp</a></div></div>)}</div><small>No email, opening hours or map details have been supplied yet.</small></div></section>
      </main>}
